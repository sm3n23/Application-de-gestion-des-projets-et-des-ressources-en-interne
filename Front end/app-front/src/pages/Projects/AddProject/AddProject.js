...
import "../EditProject/EditProject.css"; // Ensure you have the necessary CSS
import TaskModal from "../EditProject/TaskModal";
import EmployeeModal from "../EditProject/EmployeeModal";
import "@fortawesome/fontawesome-free/css/all.min.css";
import ErrorModal from "../EditProject/ErrorModal";
...
export default function AddProject() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Creating project with data:", newProject); // Debug log
      const response = await axios.post(`http://localhost:8085/projects?username=${AuthenticatedEmployee.username}`, newProject);

      if (budgetConsumption.amount) {
        const projectId = response.data.id;
        const budgetResponse = await axios.post(
          `http://localhost:8085/projects/${projectId}/budget-consumption`,
          {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex-container">
            ...
            <InputField
              ...
              value={project.totalConsumedBudget || 0}
              onChange={handleChange}
              readOnly
            />
